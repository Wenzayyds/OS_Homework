...
load_icode(int fd, int argc, char **kargv)
{
    ...
    //(3.2) read raw data content in file and resolve proghdr based on info in elfhdr
    //解析ELF头，获取文件的基本信息
    //根据ELF头中给出的程序头表偏移量，获取程序头表的位置，并解析出其中的各个段的信息。
    //这些信息将用于建立进程的虚拟地址空间，将可执行文件中的各个段映射到进程的虚拟地址空间中，从而完成进程的加载。
    ...
    //(4) call mm_map to setup user stack, and put parameters into user stack
    //调用mm_map函数建立用户栈的虚拟地址映射，同时将参数拷贝到用户栈中。
    ...
    //(5) setup current process's mm, cr3, reset pgidr (using lcr3 MARCO)
    //设置当前进程的mm、cr3和pgdir，使用lcr3宏更新CR3寄存器。
    mm_count_inc(mm);
    current->mm = mm;
    ...
    //(6) setup uargc and uargv in user stacks
    //在用户栈中设置uargc和uargv，用于传递参数给用户进程。
    ...
    *(int *)stacktop = argc;
    ...
    //(7) setup trapframe for user environment
    //设置trapframe结构体，用于用户环境中断时保存进程的执行状态。
