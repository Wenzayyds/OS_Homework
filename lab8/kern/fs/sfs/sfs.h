...
/*
 * On-disk superblock
 * 超级块：它包含了关于文件系统的所有关键参数，
 * 当计算机被启动或文件系统被首次接触时，超级块的内容就会被装入内存。
 */
struct sfs_super
{
    uint32_t magic;                  /* 内核通过它来检查镜像是否是合法的SFS img magic number, should be SFS_MAGIC */
    uint32_t blocks;                 /* SFS中block的数量 # of blocks in fs */
    uint32_t unused_blocks;          /* SFS中还没有被使用的block的数量 # of unused blocks in fs */
    char info[SFS_MAX_INFO_LEN + 1]; /* 包含了字符串”simple file system” infomation for sfs  */
};

/* inode (on disk) */
// 描述文件系统中的一个磁盘上的inode的格式
...
struct sfs_inode
{
    struct sfs_disk_inode *din; /* SFS硬盘的inode on-disk inode */
    //硬盘的inode的内容是保存在硬盘中的，只是在进程需要时才被读入到内存中，用于访问文件或目录的具体内容数据
    uint32_t ino;               /* inode number */
    bool dirty;                 /* true if inode modified */
    int reclaim_count;          /* kill inode if it hits zero */
    semaphore_t sem;            /* semaphore for din 对din进行信号量控制的信号量 */
...
struct sfs_fs
{
    struct sfs_super super;  /* on-disk superblock 超级块，存储整个文件系统的相关信息 */
